...
import models
import datasets
from loss import CLUB
from loss import compute_semantic_pos_loss, compute_alignment_loss
import datetime
from tensorboardX import SummaryWriter
from train_util import *

'''
This code is based on SCN.
'''
...
parser.add_argument('--arch', '-a', metavar='ARCH', default='CDSNet',  help='model architecture')
...
parser.add_argument('--milestones', default=[5000, 50000], metavar='N', nargs='*', help='epochs at which learning rate is divided by 2')
...
def main():
    ...
    #        flow_transforms.RandomTranspose(),
    ...
    mi_estimator = CLUB(x_dim=32, y_dim=32).cuda()
    mi_optimizer = torch.optim.Adam(mi_estimator.parameters(), lr = 0.0005) 
    for epoch in range(args.start_epoch, args.epochs):
        # train for one epoch
        train_avg_slic, train_avg_sem, iteration = train(train_loader, model, optimizer, epoch,
                                                         train_writer, spixelID, XY_feat_stack, mi_estimator, mi_optimizer)
...
def train(train_loader, model, optimizer, epoch, train_writer, init_spixl_map_idx, xy_feat, mi_estimator, mi_optimizer):
    ...
    for i, (input, label, hsv, sob) in enumerate(train_loader):
        ...
        output, prob_assit, align, mi = model(input_gpu, sob.to(device))
        ...
        slic_loss_1, _, _ = compute_semantic_pos_loss(prob_assit, LABXY_feat_tensor,
                                                                pos_weight= args.pos_weight, kernel_size=args.downsize)
        
        #cross_entropy_loss_edge( pred_edge, edge.to(device))
        
        #print(align[1].shape)
        #print(mi[0].shape)
        #print(mi[1].shape)
#       exit(0)
        sample_loss = mi_estimator(mi[0].squeeze(), mi[1].squeeze())        
        loss = slic_loss + slic_loss_1 + sample_loss + compute_alignment_loss(align)
            
        edge_loss = sample_loss
        # ========= back propagate ===============
        optimizer.zero_grad()
        adjust_learning_rate(optimizer, args.lr, iteration, args.milestones[-1] + args.additional_step, 0.9)
        loss.backward()
        optimizer.step()
        
        # ===========mi optimizer ==========================
        mi_estimator.train()
        mi_loss = mi_estimator.learning_loss(mi[0].squeeze().detach(), mi[1].squeeze().detach())
        ...
        loss_edge.update(edge_loss.item(), input_gpu.size(0))

        if i % args.print_freq == 0:
            print('train Epoch: [{0}][{1}/{2}]\t Time {3}\t Data {4}\t Total_loss {5}\t Loss_sem {6}\t Loss_pos {7}\t Loss_edge {8}'
                  .format(epoch, i, epoch_size, batch_time, data_time, total_loss, losses_sem, losses_pos, loss_edge))
    ...
    if epoch % args.record_freq == 0:
        ...
        train_writer.add_scalar('loss_Edge', edge_loss.item(), epoch)
...
def validate(val_loader, model, epoch, val_writer, init_spixl_map_idx, xy_feat):
    ...
    for i, (input, label,_, sob) in enumerate(val_loader):
