...
parser.add_argument('--pretrained', metavar='PTH', help='path to pre-trained model',
                                    default= './weights/model_best.tar')
...
def test(args, model, img_paths, save_path, idx):
    ...
    img_ = imread(load_path)#.astype(np.uint8)
 #   print(img_.shape)
 #   img_ = cv2.cvtColor(img_,cv2.COLOR_GRAY2RGB).astype(float32)
    H, W, _ = img_.shape
    ...
    imwrite(spixl_save_name, (spixel_viz.transpose(1, 2, 0) * 255.).astype(np.uint8))
...
def main():
    ...
    for n in range(len(tst_lst)):
      test(args, model, tst_lst, save_path, n)
